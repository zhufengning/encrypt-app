...
function updateCipherOptions() {
  const cipherOptions = {
    '单表替代密码': ['Caesar cipher', 'Keyword cipher', 'Affine cipher'],
  ...
  <v-container>
    ...
    <v-row>
      <v-col cols="12">
        ...
        <v-text-field v-if="selectedCipher === 'Double-Transposition cipher'" v-model="key2" label="密钥2" hint="请输入第二密钥"
        ...
        <v-text-field v-if="selectedCipher === 'ca'" v-model="carule" label="规则" hint="请输入CA规则（整数）" persistent-hint
          outlined></v-text-field>
        <v-text-field v-if="selectedCipher === 'ca'" v-model="casource" label="源位置" hint="请输入CA源位置（整数）" persistent-hint
          outlined></v-text-field>
        <!-- salsa20算法的特定输入 -->
        <v-text-field v-if="selectedCipher === 'salsa20'" v-model="key" label="密钥" hint="请输入32字节的密钥" persistent-hint
          outlined></v-text-field>
        <v-text-field v-if="selectedCipher === 'salsa20'" v-model="nonce" label="随机数" hint="请输入8字节的随机数" persistent-hint
          outlined></v-text-field>
